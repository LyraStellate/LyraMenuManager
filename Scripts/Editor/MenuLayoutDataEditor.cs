    ...
    public class MenuLayoutDataEditor : UnityEditor.Editor{
        ...
        public override void OnInspectorGUI(){
            ...
            if (_showHeader){
                ...
                var linkStyle = new GUIStyle(EditorStyles.linkLabel){
                    ...
                    imagePosition = ImagePosition.ImageLeft // アイコンとテキストを並べる
                };

                // 1. オンラインマニュアル (_Help = はてな/本マーク)
                ...
                // 2. サポートページ / GitHub (BuildSettings.Web.Small = 地球儀マーク)
            ...
            if (_showSettings){
                ...
                GUI.enabled = isEnabledProp.boolValue; // オフなら以下の設定をグレーアウト
        ...
        private static void AddAfterPlugin(SerializedProperty listProp, string qualifiedName){
            for (int i = 0; i < listProp.arraySize; i++)
                if (listProp.GetArrayElementAtIndex(i).stringValue == qualifiedName) return; // 重複防止
            ...
            try {
                ...
                foreach (nadena.dev.ndmf.ExportsPlugin attr in attrs){
                    try {
                        ...
                        if (pluginType.FullName == typeof(MenuManagerPlugin).FullName) continue; // 自身を除外
                        // Plugin<T>.Instance 静的プロパティからシングルトンを取得
                        ...
                        // フォールバック: GetUninitializedObject
                        if (instance == null){
